...
def calculate_max_batch_size(window_size, device, net):
    ...
    while True:
        try:
            ...
            dummy_input = torch.randn(batch_size, 3, window_size, window_size).to(
                device
            )
            # Forward pass
            net(dummy_input)
...
def smart_batched_sliding_window(tensor, window_size, device, net):
    ...
    with torch.no_grad():
        for i in range(0, len(x_coords), max_batch_size):
            ...
            # Print progress
            print(
                f"Processed {min(i + max_batch_size, len(x_coords))}/{len(x_coords)} windows"
            )
...
def analyze_image(image_path: str, model_path: str) -> None:
    ...
    predictions, x_coords, y_coords = smart_batched_sliding_window(
        tensor, window_size, device, net
    )
